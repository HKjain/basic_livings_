...
def payment(request):
    ...
    myUser = auth.get_user(request)
    orderid = "F{date}{id}{uid}".format(date=datetime.today().strftime('%d%m%y%H'), id=id, uid=myUser.user_id)
    usr = User.object.get(user_id=myUser.user_id)
    email = usr.get_email()
    ...
    param_dict = {
        ...
        'WEBSITE': 'DEFAULT',
        'CHANNEL_ID': 'WEB',
        'CALLBACK_URL': 'https://hkjain0609.pythonanywhere.com/food/vendor/payment/handlerequest/',
