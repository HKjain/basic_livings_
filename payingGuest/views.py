...
@login_required(login_url="/accounts/")
def manageEmail(request, kwargs=None):
...
def sendEmail(request):
    ...
    if request.method == 'POST':
        ...
        if len(recipients) == 0:
            errorMessage = "Check at least one Recipient!"
            return manageEmail(request=request, kwargs={'error': errorMessage})
        ...
        if sender and subject and message and password:
            try:
                ...
                errorMessage = "Invalid Email-Id of your account or Invalid Password!"
                return manageEmail(request=request, kwargs={'error': errorMessage})
            success = "Email(s) sent successfully!!"
            return manageEmail(request=request, kwargs={'success': success})
        else:
...
def payment(request):
    ...
    myUser = auth.get_user(request)
    orderid = "R{date}{id}{uid}".format(date=datetime.today().strftime('%d%m%y%H'), id=id, uid=myUser.user_id)
    usr = User.object.get(user_id=myUser.user_id)
    email = usr.get_email()
    ...
    param_dict = {
            ...
            'WEBSITE': 'DEFAULT',
            'CHANNEL_ID': 'WEB',
            'CALLBACK_URL': 'http://hkjain0609.pythonanywhere.com/payingGuest/vendor/payment/handlerequest/',
